...
insert into public.profiles
  (id, name, age, city, route, bio, tags, gender, location_focus, tint)
values
  ('11111111-1111-1111-1111-111111111111', 'Achol', 25, 'Melbourne', 'Lives in Melbourne · from Juba',
   'Engineering student. I make the best aseeda in Victoria. Looking for someone who still calls their mum on Sundays.',
   array['Dinka','Coffee','Afrobeats'], 'Woman', 'Diaspora', 'linear-gradient(150deg,#E0A24F,#A85A22)'),
  ('22222222-2222-2222-2222-222222222222', 'Garang', 29, 'Omaha', 'Lives in Omaha · from Bor',
   'Nurse by day. I will teach you to dance to Dynamq and lose every argument about jollof.',
   array['Family-first','Basketball','Gospel'], 'Man', 'Diaspora', 'linear-gradient(150deg,#3E8C63,#1C5436)'),
  ('33333333-3333-3333-3333-333333333333', 'Nyandeng', 27, 'Nairobi', 'Lives in Nairobi · from Juba',
   'Founder building for the continent. Diaspora kid with a Juba heart. Tea, not coffee.',
   array['Entrepreneur','Travel','Faith'], 'Woman', 'Diaspora', 'linear-gradient(150deg,#C96A4A,#7E2B22)'),
  ('44444444-4444-4444-4444-444444444444', 'Deng', 31, 'Calgary', 'Lives in Calgary · from Wau',
   'Civil engineer, quiet type. Sunday calls home to Wau are sacred. I cook a serious kisra.',
   array['Calm','Cooking','Football'], 'Man', 'Diaspora', 'linear-gradient(150deg,#4A6E96,#1E3A6B)'),
  ('55555555-5555-5555-5555-555555555555', 'Aluel', 24, 'Juba', 'Lives in Juba',
   'Teacher and stubborn reader. I want letters, not just texts. Romance me with a real plan.',
   array['Reader','Tea','Romantic'], 'Woman', 'Home', 'linear-gradient(150deg,#9C5A8C,#5A2A52)'),
  ('66666666-6666-6666-6666-666666666666', 'Mabior', 28, 'London', 'Lives in London · from Malakal',
   'DJ bringing the Juba sound to the ends. Night owl, loyal to a fault, terrible at texting back fast.',
   array['Music','Night owl','Loyal'], 'Man', 'Diaspora', 'linear-gradient(150deg,#3F8A86,#16524F)')
on conflict (id) do update set
  name = excluded.name, age = excluded.age, city = excluded.city, route = excluded.route,
  bio = excluded.bio, tags = excluded.tags, gender = excluded.gender,
